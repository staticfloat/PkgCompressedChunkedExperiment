...
# Analysis script: Compare different compression settings and scenarios
...
for (idx, (chunk_size_name, chunk_size)) in enumerate(chunk_sizes)
    ...
    for (name, reg_file) in ("old" => old_targz_path, "new" => new_targz_path)
        ...
        index(index_file, reg_file; dopts)
    end
    recompress_chunks(; dopts)
    ...
    for name in ("old", "new")
        ...
        index_file = joinpath(indexes_dir, "$(name)-$(chunk_size_name).caibx")
        chunks = load_chunk_index(index_file)
        synthesize(rreg_path, chunks, chunk_store)
    end
...
for (idx, (chunk_size_name, chunk_size)) in enumerate(chunk_sizes)
    ...
    new_chunks = load_chunk_index(index_file)
    old_chunks = load_seed_chunks(old_reg_path)
    ...
    report_stats(
        ...
        output_file;
        data_transfer = sum([filesize(chunk_path(c, chunk_store)) for c in missing_chunks])
    )
end
